...
async function checkEmailExists(email) {
  return await usersRepository.checkEmailExists(email); //fungsi ini memanggil dari repository
}

//Soal 3: Mengubah password
async function updatePassword(userId, newPassword) {
  try {
    const hashedPassword = await hashPassword(newPassword); //Melakukan hashing pada password
